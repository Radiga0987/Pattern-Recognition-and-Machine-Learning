...
def Principal_Component_Analysis(data,L):
    cov = covariance(data,np.mean(data,axis=0),0,[],[],0)
    e,V = np.linalg.eig(cov)        #Finding Eigen values and vectors of martix A
    #print("Condition number of eigenvector matrix of A : ",np.linalg.cond(V))
    mag,e,V = map(np.array, zip(*sorted(zip(abs(e),e,V.T),reverse=True)))   #Sorting eigen vals and vecs
    V=V.T                           #Transpose of V taken in previous step
    u = np.linalg.inv(V)
