...
def Synth():
    ...
    #Predict
    y_pred,scores = predict(phi,W)
    y_pred += 1
    print("Accuracy on dev data of Synthetic dataset using LR",(1-sum(abs(y_pred-dev_data[:,2]))/len(dev_data))*100,"%")
...
def Image_Dataset():
    ...
    #Predict
    dev = np.hstack((np.ones((len(dev),1)),dev))
    y_pred,scores = predict(dev,W)
    y_pred += 1
    y_pred = np.array(y_pred)
    
    print("Accuracy on dev data of image dataset using LR",(1-np.count_nonzero(y_pred-dev_labels)/len(dev_labels))*100,"%")
    ...
    ROC_DET([scores],dev_labels,"Image Dataset")
...
def spoken_digits():
    ...
    #Perform Lda
    Q = lda_(train_extended,train_labels,0.9)
    ...
    #Train
    W = logistic_reg(train_extended,train_labels,1000,0.8,0.1)

    #Predict
    y_pred,scores = predict(dev_extended,W)
    y_pred += 1
    y_pred = np.array(y_pred)
    
    print("Accuracy on dev data of Isolated Digits using LR",(1-np.count_nonzero(y_pred-dev_labels)/len(dev_labels))*100,"%")
...
def Handwriting_Data():
    ...
    #Lda
    Q = lda_(train_extended,train_labels,0.99)
    train_extended = transform(Q,train_extended)
    dev_extended = transform(Q,dev_extended)
    ...
    print("Accuracy on dev data of Telugu characters using LR",(1-np.count_nonzero(y_pred-dev_labels)/len(dev_labels))*100,"%")
